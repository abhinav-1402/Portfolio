      ...
      {options.map(opt => {
        ...
        return (
          <button
            ...
            onMouseOver={e => { if (!active) e.currentTarget.style.borderColor = accent; }}
            onMouseOut={e =>  { if (!active) e.currentTarget.style.borderColor = border; }}
          >
            {opt.label}
          </button>
        );
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{
          display: 'flex', justifyContent: 'space-between',
          fontFamily: mono, fontSize: 8, color: muted, marginTop: 4,
        }}>
          <span>tap</span><span>power</span><span>rocket</span>
        </div>
      </div>
